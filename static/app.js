...
function showNodeDesign() {
    var query1 = d3.select('input#baseconcept_textinput').data()[0];
    var label1 = NODE_IdToObj(query1).label;
    var query2 = d3.select('input#additiveconcept_textinput').data()[0];
    var label2 = NODE_IdToObj(query2).label;
    console.log(query1,label1,query2,label2)
    d3.json("/nodetodesign/" + JSON.stringify({"node1_id": label1, "node2_id": label2}), function(error, data) {
        if (error) {
            console.log(error);
        } else {
            // console.log(data["generation"]);
            var explanationElement = d3.select("#explanation");
            explanationElement.text(JSON.stringify("Explanation: "+data["explanation"]));
            var descriptionElement = d3.select("#description");
            descriptionElement.text(JSON.stringify("Description: "+data["description"]));
        };
    });
};
...
function node_left_click_on(){
      ...
      GRAPH.selectAll('.gnode').on('click',function(d){
          ...
          FOCUSING_NODE = d.wid;
          if( d3.select('#AI').style('display')=='block'){
              if(preNode == clicked_data.wid || d3.select('input#baseconcept_textinput').data()[0] == d3.select('input#additiveconcept_textinput').data()[0]){
                  //highlight
                  var highlights = {'nodes':[clicked_data.wid],'paths':[],'paths1':[]};
                  highlight_nodespaths(highlights);
                  //update searchbox
                  d3.select('input#baseconcept_textinput').node().value = clicked_data.label;
                  d3.select('input#additiveconcept_textinput').node().value='';
                  //attach data
                  d3.select('input#baseconcept_textinput').data([clicked_data.wid]);
                  d3.select('input#additiveconcept_textinput').data([null]);
              }else{
                  //highlight
                  var highlights = {'nodes':[clicked_data.wid,preNode],'paths':[],'paths1':[]};
                  highlight_nodespaths(highlights);

                  d3.selectAll('input#baseconcept_textinput,input#additiveconcept_textinput')
                    .each(function(d){
                        if(d==preNode){
                            //update search box
                            this.value = preLabel;
                  ...
                  // Show_FuncPanel('baseconcept_textinput');
              };
