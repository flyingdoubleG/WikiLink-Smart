...
class UndirectedG(object):
    ...
    def get_Rel_one(self, ipt, tp, minhops, localnodes=None):
        ...
        listB = PathBuffer()
        prev_path = None
        ignore_nodes = set() # node yielded before
        prob = 0.5
        ignore_probs = []
        probs_len = 6
        for i in range(1, probs_len):
            ignore_probs.append(prob ** (probs_len - i))
        while True:
            if not prev_path:
                length, path = explore_func(G, ipt, tp, minhops)
                listB.push(length, path)
            else:
                # 每次搜索路径里的词不重复
                for nodeth, node in enumerate(prev_path):
                    # if random.random() < ignore_probs[nodeth]:
                    ignore_nodes.add(node)
                try:
                    length, path = explore_func(G, ipt, tp, minhops, ignore_nodes)
                    listB.push(length, path)
...
# -----------------------functions-----------------------------------------
def explore_func(G, ipt, tp, minhops, ignore_nodes=None):
    if ignore_nodes:
        ignore_nodes = set(ignore_nodes)
        if ipt in ignore_nodes:
            ignore_nodes.remove(ipt)

    Gneighbor = G.neighbors_iter
    ...
    if ignore_nodes:
        ...
        Gneighbor = filter_iter(Gneighbor)

    push = heappush
    pop = heappop
    dist = {}  # dictionary of final distances (shortest distance from start node to the node)
    seen = {ipt: 0}
    c = itertools.count()
    fringe = []  # use heapq with (distance,label) tuples (distance from start node to the node)
    push(fringe, (0, next(c), ipt))  # every element: (distance, index, nodeId)
    paths = {ipt: [ipt]}  # (path from start node to the node)
    while fringe:
        ...
        dist[v] = d  # shortest distance from start to v
        # print("fringe: ", fringe)
        # print("paths: ", paths)
        ...
        for u in Gneighbor(v):
            if ignore_nodes is not None and u in ignore_nodes:
                continue
